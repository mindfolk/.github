...
# we use the counter to not get rate limited by ME, we send a request roughly every ~35s
counter = 35
# able to pull from 20 up to 1000, I mostly use the latest 40 activities (listing/bids/sales)
limit = 40
...
→ {mint_token}"
                # use this line, 158, if you run the script on a windows server, otherwise it's not needed
